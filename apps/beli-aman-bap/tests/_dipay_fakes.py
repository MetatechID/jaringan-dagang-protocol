"""Shared fakes for Dipay payment gateway unit tests.

Centralises the SQLAlchemy-result + Cart/Brand/Order stub helpers used by
``tests/test_dipay_invoices.py`` and ``tests/test_dipay_disbursements.py``.
Import from here in both files — no inline duplicates.

Mirror of ``tests/_sento_fakes.py`` with dipay_* defaults instead of
sento_*.
"""

from __future__ import annotations

import os
import sys
from types import SimpleNamespace

# Make the BAP package importable so ``from services import dipay_invoices``
# works from any test file under ``tests/``. One place, both files.
_BAP_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
if _BAP_DIR not in sys.path:
    sys.path.insert(0, _BAP_DIR)


class FakeExecute:
    """Async SQL result shim supporting both consumer styles."""

    def __init__(self, value):
        self._value = value

    def scalars(self):
        return SimpleNamespace(first=lambda: self._value)

    def scalar_one_or_none(self):
        return self._value


class FakeSession:
    """Async session whose ``execute`` returns results from a queue."""

    def __init__(self, results):
        self._results = list(results)
        self.committed = False
        self.added: list = []

    async def execute(self, *_args, **_kwargs):
        nxt = self._results.pop(0)
        if isinstance(nxt, Exception):
            raise nxt
        return FakeExecute(nxt)

    async def commit(self):
        self.committed = True

    def add(self, obj):
        self.added.append(obj)
        return None


def StubCart(*, payment_state: str = "pending", status=None, **overrides):
    """Minimal Cart-shape. Pass overrides for any extra attribute."""
    if status is None:
        from models.bot_rest import CartStatus
        status = CartStatus.CONFIRMED
    base = dict(
        id="cart-123",
        bpp_id="safiya.bpp.jaringan-dagang.id",
        order_id="order-abc",
        payment_state=payment_state,
        status=status,
        invoice_id=None,
        invoice_provider=None,
        qr_image_url=None,
        qris_image_url=None,
        qris_content=None,
        quote_json={"total_idr": 100_000},
    )
    base.update(overrides)
    return SimpleNamespace(**base)


def StubBrand(
    *,
    payment_provider: str = "dipay",
    dipay_client_key: str = "brand-client-key",
    dipay_client_secret: str = "brand-client-secret",
    dipay_private_key_b64: str = "",
    dipay_merchant_id: str = "M-123",
    dipay_disbursement_bank_code: str = "014",
    dipay_disbursement_bank_account: str = "1234567890",
    dipay_disbursement_holder_name: str = "Safiya",
    slug: str = "safiya",
    bpp_id: str = "safiya.bpp.jaringan-dagang.id",
    **overrides,
) -> SimpleNamespace:
    """Minimal Brand-shape with sane defaults for Dipay-happy-path tests."""
    base = dict(
        id="brand-id",
        slug=slug,
        name="Safiya",
        bpp_id=bpp_id,
        payment_provider=payment_provider,
        dipay_client_key=dipay_client_key,
        dipay_client_secret=dipay_client_secret,
        dipay_private_key_b64=dipay_private_key_b64,
        dipay_merchant_id=dipay_merchant_id,
        dipay_disbursement_bank_code=dipay_disbursement_bank_code,
        dipay_disbursement_bank_account=dipay_disbursement_bank_account,
        dipay_disbursement_holder_name=dipay_disbursement_holder_name,
    )
    base.update(overrides)
    return SimpleNamespace(**base)


def StubOrder(*, total_idr: int = 750_000, **overrides) -> SimpleNamespace:
    """Minimal Order-shape for create_invoice_for_order tests."""
    base = dict(
        id="order-test-id",
        brand_id="brand-id",
        total_idr=total_idr,
        items=[{"name": "Test", "qty": 1, "unit_price_idr": total_idr}],
        shipping_address={"email": "buyer@example.com", "recipient_name": "Buyer"},
        payment_method_snapshot=None,
    )
    base.update(overrides)
    return SimpleNamespace(**base)
